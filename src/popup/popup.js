...
async function loadEnabledLabels() { // 🔁 CHANGED: now sets enabledLabels safely with defaults
...
async function saveEnabledLabels() { // 🔁 CHANGED: broadcast to ALL tabs, show status
    await chrome.storage.sync.set({ [SETTINGS_KEY]: { enabledLabels } });

    const tabs = await chrome.tabs.query({}); // 🔁 CHANGED: all tabs, not only active
    await Promise.all(
      tabs.map((t) =>
        t.id
