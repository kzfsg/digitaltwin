...
function showInOverlay(text, field) {
    ...
    content.innerHTML = `
        <div style="margin-bottom: 8px;">
            ...
            <strong>Text:</strong> ${text.substring(0, 100)}${text.length > 100 ? '...' : ''}
        </div>
    `;
...
function getFieldDescription(field) {
    ...
    const placeholder = field.placeholder || field.getAttribute('aria-label') || '';
...
function attachChatbotListener(field) {
    ...
    const sendButtons = document.querySelectorAll('button[type="submit"], button[aria-label*="Send"], button[title*="Send"], [data-testid*="send"]');
    sendButtons.forEach(button => {
        if (!button._sendListenerAttached) {
            ...
            button.addEventListener('click', () => {
                const text = field.value || field.textContent || '';
                if (text.trim().length > 0) {
                    console.log('🚨 Text about to be sent to AI chatbot via button:', text);
                    showInOverlay(`[SENDING] ${text}`, field);
                }
            });
        }
...
function scanChatbotInputs(root = document) {
    ...
    const generalInputs = root.querySelectorAll('input[type="text"], textarea, div[contenteditable="true"]');
